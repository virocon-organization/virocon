...
def exp_weibull_reference_data():
    with np.load(
        "tests/comparison-to-virocon-v1/reference_data/distributions/reference_data_exp_weibull.npz"
    ) as data:
        data_dict = dict(data)
    return data_dict
...
def exp_weibull_reference_data_wlsq_fit():
    with np.load(
        "tests/comparison-to-virocon-v1/reference_data/distributions/reference_data_exp_weibull_wlsq_fit.npz"
    ) as data:
        data_dict = dict(data)
    return data_dict
