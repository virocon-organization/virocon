...
def reference_coordinates_IFORM():
    with np.load(
        "tests/comparison-to-virocon-v1/reference_data/contours/reference_data_IFORM.npz"
    ) as data:
...
def reference_coordinates_ISORM():
    with np.load(
        "tests/comparison-to-virocon-v1/reference_data/contours/reference_data_ISORM.npz"
    ) as data:
        ref_coordinates = data["ref_coordinates"]
    return ref_coordinates
...
def reference_coordinates_HDC():
    with np.load(
        "tests/comparison-to-virocon-v1/reference_data/contours/reference_data_HDC.npz"
    ) as data:
        ref_coordinates = data["ref_coordinates"]
    return ref_coordinates
...
def reference_data_DSContour():
    with np.load(
        "tests/comparison-to-virocon-v1/reference_data/contours/reference_data_DSContour.npz"
    ) as data:
        data_dict = dict(data)
    return data_dict
...
def test_DirectSamplingContour(reference_data_DSContour):
    ...
    np.testing.assert_allclose(my_coordinates, ref_coordinates, rtol=0.05)
